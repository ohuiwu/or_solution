...
# create the test dataset to run the result
def create_test_data():
    ...
    data = {'name': name_list, 'value': test}
    df = pd.DataFrame(data)

    # To display the resulting DataFrame
    print(df)
    return test, name_list, df
# operation research function
def max_pair_calcilate(test, name_list):
    ...
    return result, dat, name_list
#output manipulate
def output_manipulate(result, dat, dat_name_list):
    ...
    result = (result
            .merge(dat_name_list, on='num_volumn', how='left')
    )

    output_result = (result
                    .query('value > 0')
                    .sort_values(['num_pair', 'num_volumn'])
                    .loc[:, ['num_pair', 'volume', 'volume_name']]
                    .rename(columns={'volume_name':'name', "volume":'value'})
                    )

    output_result = (df
                    .merge(output_result, on=['name', 'value'], how='left')
                    .sort_values(by='num_pair')
                    )
    return output_result


if __name__ == '__main__':
    result, dat, dat_name_list = max_pair_calcilate(test, name_list)
    test, name_list, df = create_test_data()
    output = output_manipulate(result, dat, dat_name_list)
    print(output)
